...
public class ResourceExceptionHandler {
	 ...
	 * HttpServletRequest request) { String error = "Database error"; HttpStatus
	 * status = HttpStatus.BAD_REQUEST; StandardError err = new
	 * StandardError(Instant.now(), status.value(), error, e.getMessage(),
	 * request.getRequestURI()); return ResponseEntity.status(status).body(err); }
	 */
}
